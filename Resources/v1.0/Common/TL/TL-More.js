...
TL.win = function(config){
	this.config = {
		type: 1, //对话框类型(可选参数,默认值为1) 1:提示 2:警告 3:正确或成功 4:错误 5:问号 6:自定义 7:iframe加载网页
		width: 400, //对话框宽度(可选参数,默认为400)
		height: "", //type设为6-7有效,这个height是指对话框去除标题栏剩余部分的高度(可选参数,默认是根据里面内容自动增长高度)
		position: [], //对话框位置position[left,top](可选参数,默认居中)
		skin: "default", //皮肤的名称(可选参数,默认为"default")
		title: "", //标题文字
		msg: "", //提示信息 type设为1-5有效
		html: "", //自定内容 type设为6有效
		url: "", //网页地址 type设为7有效
		closeEvent: null, //type设为1-7时,关闭按钮的回调函数 type设为1-4时,确定铵钮的回调函数 type设为5时,取消按钮的回调函数(可选参数)
	...
	show: function(){
		...
		if (config.type > 0 && config.type <= 5) {
			...
			if (config.type == 6) {
				...
				winMiddle.innerHTML = "<iframe src=\"" + config.url + "\" width=\"100%\" height=\"" + config.height + "\"></iframe>";
			}
		}
